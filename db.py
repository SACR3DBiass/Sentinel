...
# ============================================================================
# FINANCIAL TRACKING (Subscription History, CAC, Churn)
# ============================================================================

def subscription_record_event(org_id: str, event_type: str, plan_name: str = "free",
                               monthly_amount: float = 0, billing_cycle: str = "monthly",
                               payment_status: str = "paid", stripe_subscription_id: str = "",
    ...
    try:
        result = sb.table("subscription_history").insert(entry).execute()
        return result.data[0] if result.data else entry
    except Exception as e:
        print(f"[SENTINEL] subscription_record_event error: {e}", flush=True)
        return None

def subscription_list_org(org_id: str, limit: int = 100) -> list:
    """List subscription history for an org."""
    sb = get_supabase()
    if not sb:
        return []
    try:
        result = sb.table("subscription_history").select("*").eq("org_id", org_id).order("occurred_at", desc=True).limit(limit).execute()
        return result.data or []
    except Exception:
        return []

def subscription_get_current(org_id: str) -> Optional[dict]:
    """Get the most recent subscription event for an org."""
    events = subscription_list_org(org_id, limit=1)
    return events[0] if events else None

def org_metrics_record(org_id: str, period_start: str, period_end: str,
                        mrr: float = 0, cac: float = 0, ltv: float = 0,
                        churn_risk_score: float = 0, is_churned: bool = False,
                        days_since_signup: int = 0, total_emails_analyzed: int = 0,
                        total_threats_blocked: int = 0, sessions_in_period: int = 0,
                        last_active_at: str = None, notes: str = "") -> Optional[dict]:
    ...
    try:
        result = sb.table("org_metrics").insert(entry).execute()
        return result.data[0] if result.data else entry
    except Exception as e:
        print(f"[SENTINEL] org_metrics_record error: {e}", flush=True)
        return None

def org_metrics_list_org(org_id: str, limit: int = 12) -> list:
    """List metrics snapshots for an org (last N months)."""
    sb = get_supabase()
    if not sb:
        return []
    try:
        result = sb.table("org_metrics").select("*").eq("org_id", org_id).order("period_start", desc=True).limit(limit).execute()
        return result.data or []
    except Exception:
        return []

def calculate_cac(org_id: str) -> float:
    """Calculate Customer Acquisition Cost for an org.
    CAC = Total marketing/sales spend / Number of conversions.
    Simplified: we use subscription revenue as proxy."""
    sub = subscription_get_current(org_id)
    if not sub:
        return 0
    monthly = float(sub.get("monthly_amount", 0))
    org = org_get(org_id)
    if not org:
        return 0
    ...
    if days <= 0:
        return monthly
    total_revenue = monthly * (days / 30)
    return round(total_revenue, 2) if total_revenue > 0 else 0

def calculate_churn_risk(org_id: str) -> float:
    """Calculate churn risk score (0-100) based on activity and subscription signals."""
    score = 0
    sub = subscription_get_current(org_id)
    if not sub:
        return 100
    event = sub.get("event_type", "")
    if event == "cancellation":
        return 100
    if event == "trial_end":
        score += 40
    metrics = org_metrics_list_org(org_id, limit=3)
    if not metrics:
        score += 30
    else:
        latest = metrics[0]
        if latest.get("sessions_in_period", 0) == 0:
            score += 35
        if latest.get("total_emails_analyzed", 0) == 0:
            score += 25
        if len(metrics) >= 2:
            prev = metrics[1]
            if latest.get("total_emails_analyzed", 0) < prev.get("total_emails_analyzed", 0) * 0.5:
                score += 20
    return min(score, 100)

def export_financial_csv(org_ids: list = None) -> str:
    """Export subscription and churn data as CSV for all orgs or specific ones."""
    import csv
    import io
    sb = get_supabase()
    output = io.StringIO()
    ...
    writer.writerow([
        "org_id", "org_name", "plan_name", "monthly_amount", "billing_cycle",
        "payment_status", "event_type", "occurred_at", "cac", "ltv",
        "churn_risk_score", "is_churned", "days_since_signup",
        "total_emails_analyzed", "total_threats_blocked", "sessions_in_period",
    ])
    if not sb:
        return output.getvalue()
    try:
        query = sb.table("subscription_history").select("*").order("occurred_at", desc=True)
        if org_ids:
            query = query.in_("org_id", org_ids)
        subs = query.limit(1000).execute().data or []
    except Exception:
        subs = []
    for sub in subs:
        oid = sub.get("org_id", "")
        org = org_get(oid) if oid else None
        org_name = org.get("name", "") if org else ""
        metrics = org_metrics_list_org(oid, limit=1)
        m = metrics[0] if metrics else {}
        writer.writerow([
            oid, org_name, sub.get("plan_name", ""),
            sub.get("monthly_amount", 0), sub.get("billing_cycle", ""),
            sub.get("payment_status", ""), sub.get("event_type", ""),
            sub.get("occurred_at", ""),
            m.get("cac", 0), m.get("ltv", 0),
            m.get("churn_risk_score", 0), m.get("is_churned", False),
            m.get("days_since_signup", 0),
            m.get("total_emails_analyzed", 0), m.get("total_threats_blocked", 0),
            m.get("sessions_in_period", 0),
        ])
    return output.getvalue()
