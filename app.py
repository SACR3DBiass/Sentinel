...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#0a0a0a;--card:#111;--border:#1e1e1e;--red:#DC2626;--green:#22c55e;
  --yellow:#eab308;--blue:#3b82f6;--purple:#a855f7;--text:#f5f5f5;--dim:#888;
  --font:'Inter',sans-serif;--mono:'JetBrains Mono',monospace;--radius:8px;
  --btn-radius:6px;
}
body{background:var(--bg);color:var(--text);font-family:var(--font);min-height:100vh;overflow-x:hidden}
a{color:var(--text);text-decoration:none}

.topbar{position:fixed;top:0;left:0;right:0;height:56px;background:var(--card);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 24px;z-index:1000;gap:16px}
.topbar-logo{display:flex;align-items:center;gap:10px}
.topbar-logo svg{width:32px;height:36px;flex-shrink:0}
.topbar-title{font-weight:700;font-size:18px;letter-spacing:2px}
.topbar-page{color:var(--dim);font-size:14px;padding-left:16px;border-left:1px solid var(--border)}
.topbar-spacer{flex:1}
.topbar-link{color:var(--dim);font-size:13px;cursor:pointer;transition:color .2s}
.topbar-link:hover{color:var(--text)}
.topbar-link.active{color:var(--text)}

.main{padding-top:72px;padding-left:24px;padding-right:24px;padding-bottom:40px;max-width:1200px;margin:0 auto}

.tabs-row{display:grid;grid-template-columns:repeat(9,1fr);gap:6px;margin-bottom:24px}
.tab-btn{background:var(--card);border:1px solid var(--border);color:var(--dim);padding:10px 8px;border-radius:var(--btn-radius);font-family:var(--font);font-size:12px;font-weight:500;cursor:pointer;transition:all .2s;text-align:center;white-space:nowrap}
.tab-btn:hover{color:var(--text);border-color:#333}
.tab-btn.active{color:#fff;background:var(--red);border-color:var(--red)}
.tab-icon{display:block;font-size:10px;letter-spacing:1px;text-transform:uppercase;margin-bottom:2px;font-weight:600}
...
.card-subtitle{font-size:13px;color:var(--dim);margin-top:4px}

.btn{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border-radius:var(--btn-radius);font-family:var(--font);font-size:13px;font-weight:500;border:1px solid #2a2a2a;background:#111;color:var(--text);cursor:pointer;transition:all .2s}
...
.input{width:100%;padding:8px 12px;background:#0d0d0d;border:1px solid var(--border);border-radius:var(--btn-radius);color:var(--text);font-family:var(--font);font-size:13px;outline:none;transition:border-color .2s}
.input:focus{border-color:var(--red)}
textarea.input{min-height:80px;resize:vertical}
select.input{cursor:pointer;appearance:auto}
.label{display:block;font-size:12px;font-weight:500;color:var(--dim);margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px}
...
.conn-item{display:flex;align-items:center;gap:14px;padding:14px 0;border-bottom:1px solid var(--border)}
...
.conn-detail{font-size:12px;color:var(--dim);margin-top:2px;font-family:var(--mono)}
.conn-actions{display:flex;gap:6px;flex-shrink:0}

.scan-item{padding:12px 0;border-bottom:1px solid var(--border)}
.scan-item:last-child{border-bottom:none}
.scan-top{display:flex;align-items:center;gap:10px;margin-bottom:6px}
.scan-meta{font-size:12px;color:var(--dim)}
.scan-stats{display:flex;gap:16px;font-size:12px;color:var(--dim);margin-top:4px}

.member-item{display:flex;align-items:center;gap:14px;padding:12px 0;border-bottom:1px solid var(--border)}
.member-item:last-child{border-bottom:none}
.member-avatar{width:36px;height:36px;border-radius:50%;background:#2a2a2a;display:flex;align-items:center;justify-content:center;font-weight:600;font-size:14px;flex-shrink:0;color:var(--red)}
.member-info{flex:1}
.member-name{font-weight:500;font-size:14px}
.member-email{font-size:12px;color:var(--dim)}
.member-role{font-size:12px;color:var(--dim);margin-top:2px}

.invite-item{display:flex;align-items:center;gap:14px;padding:12px 0;border-bottom:1px solid var(--border)}
.invite-item:last-child{border-bottom:none}

.rule-item{display:flex;align-items:center;gap:14px;padding:12px 0;border-bottom:1px solid var(--border)}
.rule-item:last-child{border-bottom:none}
.rule-info{flex:1;min-width:0}
.rule-name{font-weight:500;font-size:14px}
.rule-pattern{font-family:var(--mono);font-size:12px;color:var(--dim);margin-top:2px;word-break:break-all}
.rule-badges{display:flex;gap:6px;margin-top:6px}
.rule-toggle{position:relative;width:40px;height:22px;flex-shrink:0}
.rule-toggle input{opacity:0;width:0;height:0}
.rule-toggle .slider{position:absolute;inset:0;background:#333;border-radius:11px;cursor:pointer;transition:.3s}
.rule-toggle .slider::before{content:'';position:absolute;width:16px;height:16px;left:3px;bottom:3px;background:#666;border-radius:50%;transition:.3s}
.rule-toggle input:checked+.slider{background:var(--red)}
.rule-toggle input:checked+.slider::before{transform:translateX(18px);background:#fff}

.toggle-row{display:flex;align-items:center;gap:12px;padding:10px 0}
.toggle-row label{font-size:13px;min-width:120px}

.toggle-switch{position:relative;width:40px;height:22px;flex-shrink:0}
.toggle-switch input{opacity:0;width:0;height:0}
.toggle-switch .slider{position:absolute;inset:0;background:#333;border-radius:11px;cursor:pointer;transition:.3s}
.toggle-switch .slider::before{content:'';position:absolute;width:16px;height:16px;left:3px;bottom:3px;background:#666;border-radius:50%;transition:.3s}
...
.stat-value{font-size:28px;font-weight:700;color:var(--red);margin-bottom:4px}
.stat-label{font-size:12px;color:var(--dim);text-transform:uppercase;letter-spacing:.5px}

.sim-item{padding:14px 0;border-bottom:1px solid var(--border)}
.sim-item:last-child{border-bottom:none}
.sim-top{display:flex;align-items:center;gap:10px;margin-bottom:6px}
.sim-stats{display:flex;gap:14px;font-size:12px;color:var(--dim);margin-top:6px}
...
.modal{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;width:100%;max-width:520px;max-height:85vh;overflow-y:auto}
...
.toast{padding:12px 20px;border-radius:var(--btn-radius);font-size:13px;font-weight:500;animation:toastIn .3s ease;min-width:240px;display:flex;align-items:center;gap:8px}
...
.spinner{display:inline-block;width:16px;height:16px;border:2px solid rgba(255,255,255,.2);border-top-color:var(--red);border-radius:50%;animation:spin .6s linear infinite}
.spinner-lg{width:32px;height:32px;border-width:3px}
@keyframes spin{to{transform:rotate(360deg)}}
.loading-center{display:flex;align-items:center;justify-content:center;padding:40px;gap:10px;color:var(--dim);font-size:13px}

.expandable-header{display:flex;align-items:center;gap:8px;cursor:pointer;padding:10px 0;color:var(--dim);font-size:13px;user-select:none}
.expandable-header:hover{color:var(--text)}
.expandable-header .arrow{transition:transform .2s;display:inline-block}
.expandable-header.open .arrow{transform:rotate(90deg)}
.expandable-body{display:none;padding:12px 0;font-size:13px;color:var(--dim);line-height:1.7}
.expandable-body.open{display:block}

.preview-box{width:100%;padding:20px;border:1px dashed var(--border);border-radius:var(--radius);text-align:center;margin-top:12px}
.preview-box img{max-height:60px;max-width:200px;object-fit:contain}

.color-input-wrap{display:flex;align-items:center;gap:10px}
.color-input-wrap input[type="color"]{width:40px;height:32px;border:1px solid var(--border);border-radius:4px;background:transparent;cursor:pointer;padding:2px}
.color-input-wrap .color-val{font-family:var(--mono);font-size:12px;color:var(--dim)}
...
<div class="topbar">
  <div class="topbar-logo">
    <svg viewBox="0 0 40 44" fill="none" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="lgS" x1="0" y1="0" x2="40" y2="44">
          <stop offset="0%" stop-color="#DC2626"/>
          <stop offset="100%" stop-color="#991b1b"/>
        </linearGradient>
      </defs>
      <path d="M20 2L4 12v16l16 14 16-14V12L20 2z" stroke="url(#lgS)" stroke-width="2.5" fill="none"/>
      <path d="M18 14l-4 8h6l-2 10 8-10h-5l4-8-7 2 5-6-6 4z" fill="url(#lgS)"/>
    </svg>
    <span class="topbar-title">SENTINEL</span>
  </div>
  <span class="topbar-page">Settings</span>
  <span class="topbar-spacer"></span>
  <a class="topbar-link" href="/dashboard" id="backLink">Back to Dashboard</a>
  <span class="topbar-link" data-action="logout">Logout</span>
</div>

<div class="main">
  <div class="tabs-row">
    <button class="tab-btn active" data-tab="connections"><span class="tab-icon">[C]</span>Email Connections</button>
    <button class="tab-btn" data-tab="scans"><span class="tab-icon">[S]</span>Scan History</button>
    <button class="tab-btn" data-tab="team"><span class="tab-icon">[T]</span>Team</button>
    <button class="tab-btn" data-tab="reports"><span class="tab-icon">[R]</span>Reports</button>
    <button class="tab-btn" data-tab="branding"><span class="tab-icon">[B]</span>Branding</button>
    <button class="tab-btn" data-tab="rules"><span class="tab-icon">[L]</span>Rules</button>
    <button class="tab-btn" data-tab="alerts"><span class="tab-icon">[A]</span>Alerts</button>
    <button class="tab-btn" data-tab="intel"><span class="tab-icon">[I]</span>Intel</button>
    <button class="tab-btn" data-tab="simulations"><span class="tab-icon">[X]</span>Simulations</button>
  </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Manage IMAP connections for phishing email scanning</div>
        </div>
        <button class="btn btn-red" data-action="open-add-connection">+ Add Connection</button>
      </div>
      <div id="connections-list"><div class="loading-center"><div class="spinner"></div>Loading connections...</div></div>
    </div>
  </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Previous and ongoing email scans</div>
        </div>
      </div>
      <div id="scans-list"><div class="loading-center"><div class="spinner"></div>Loading scan history...</div></div>
    </div>
  </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Manage who has access to SENTINEL</div>
        </div>
      </div>
      <div id="team-members"><div class="loading-center"><div class="spinner"></div>Loading team...</div></div>
    </div>
    <div class="card">
      <div class="card-header">
        <div>
          <div class="card-title">Pending Invites</div>
          <div class="card-subtitle">Invite new members by email</div>
        </div>
      </div>
      <form id="invite-form" style="margin-bottom:16px">
        <div class="form-row">
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="input" name="role">
              <option value="friend">Friend Lite</option>
              <option value="member">Member Full</option>
            </select>
          </div>
        </div>
        <button class="btn btn-red" type="submit">Send Invite</button>
      </form>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Configure automated summary reports</div>
        </div>
      </div>
      <div id="reports-content"><div class="loading-center"><div class="spinner"></div>Loading reports...</div></div>
    </div>
  </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Customize appearance for your organization</div>
        </div>
      </div>
      <form id="branding-form">
        <div class="form-row">
          <div class="form-group">
            <label class="label">Organization Display Name</label>
            <input class="input" type="text" name="org_display_name" placeholder="My Company">
          </div>
          <div class="form-group">
            <label class="label">Logo URL</label>
            <input class="input" type="url" name="logo_url" placeholder="https://example.com/logo.png">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            ...
            <div class="color-input-wrap">
              <input type="color" name="primary_color" value="#DC2626">
              <span class="color-val" id="primary-color-val">#DC2626</span>
            </div>
          </div>
          <div class="form-group">
            <label class="label">Secondary Color</label>
            <div class="color-input-wrap">
              <input type="color" name="secondary_color" value="#111111">
              <span class="color-val" id="secondary-color-val">#111111</span>
            </div>
          </div>
        </div>
        <div class="form-group">
          <label class="label">Custom CSS</label>
          <textarea class="input" name="custom_css" placeholder=".custom-class { color: red; }"></textarea>
        </div>
        <div style="display:flex;gap:8px">
          <button class="btn btn-red" type="submit">Save Branding</button>
          <button class="btn" type="button" data-action="reset-branding">Reset to Default</button>
        </div>
      </form>
      <div class="preview-box" id="branding-preview" style="display:none">
        <div style="margin-bottom:8px;font-size:12px;color:var(--dim)">PREVIEW</div>
        <img id="branding-preview-img" src="" alt="Logo preview">
      </div>
    </div>
  </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Create and manage phishing detection rules</div>
        </div>
      </div>
      <form id="rule-form" style="margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border)">
        <div class="form-row-3">
          <div class="form-group">
            <label class="label">Rule Name</label>
            <input class="input" type="text" name="name" placeholder="e.g. Bank Phish" required>
          </div>
          <div class="form-group">
            <label class="label">Pattern</label>
            <input class="input" type="text" name="pattern" placeholder="e.g. secure-login" required>
          </div>
          <div class="form-group">
        ...
        <div class="form-row">
          ...
          <div class="form-group">
            ...
            <input class="input" type="text" name="description" placeholder="Optional description">
          </div>
        </div>
        <button class="btn btn-red" type="submit">Create Rule</button>
      </form>
      <div id="rules-list"><div class="loading-center"><div class="spinner"></div>Loading rules...</div></div>
    </div>
    <div class="card">
      <div class="card-header">
        <div>
          <div class="card-title">Test Rules</div>
          <div class="card-subtitle">Check text against your active rules</div>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <input class="input" type="text" id="rule-test-input" placeholder="Enter text to test...">
        </div>
        <div class="form-group">
          <button class="btn" data-action="test-rules">Run Test</button>
        </div>
      </div>
    ...
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">Set up notifications for detected threats</div>
        </div>
      </div>
      <div id="alerts-content"><div class="loading-center"><div class="spinner"></div>Loading alert config...</div></div>
    </div>
  </div>
  ...
  <div class="panel" id="panel-intel">
    <div class="card">
      <div class="card-header">
        <div>
          ...
          <div class="card-subtitle">View indicators and submit new intel</div>
        </div>
      </div>
      <div id="intel-stats"></div>
      <div style="border-top:1px solid var(--border);padding-top:16px;margin-top:8px">
        <div class="card-title" style="margin-bottom:12px">Submit Indicator</div>
        <form id="intel-form">
          <div class="form-row-3">
            <div class="form-group">
              <label class="label">Indicator</label>
              <input class="input" type="text" name="indicator" placeholder="e.g. evil-phish.com" required>
            </div>
            <div class="form-group">
              <label class="label">Type</label>
              <select class="input" name="indicator_type">
                <option value="domain">Domain</option>
            ...
            <div class="form-group">
              ...
              <select class="input" name="category">
                ...
                <option value="phishing_sender">Phishing Sender</option>
    ...
    <div class="card">
      <div class="card-header">
        <div class="card-title">Intel Feed</div>
      </div>
      <div id="intel-feed"><div class="loading-center"><div class="spinner"></div>Loading feed...</div></div>
    </div>
  </div>

  <!-- SIMULATIONS -->
  <div class="panel" id="panel-simulations">
    <div class="card">
      <div class="card-header">
        <div>
          <div class="card-title">Phishing Simulations</div>
          <div class="card-subtitle">Create and manage training simulations</div>
        </div>
      </div>
      <form id="sim-form" style="margin-bottom:20px;padding-bottom:16px;border-bottom:1px solid var(--border)">
        <div class="form-row">
          <div class="form-group">
            <label class="label">Campaign Name</label>
            <input class="input" type="text" name="name" placeholder="e.g. Q3 Security Test" required>
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
          <label class="label">Target User IDs (comma separated)</label>
          <input class="input" type="text" name="target_user_ids" placeholder="e.g. user1, user2, user3">
        </div>
        <button class="btn btn-red" type="submit">Create Simulation</button>
      </form>
      <div id="sim-list"><div class="loading-center"><div class="spinner"></div>Loading simulations...</div></div>
    </div>
  </div>
</div>

<!-- ADD CONNECTION MODAL -->
<div class="modal-overlay" id="modal-add-connection">
  <div class="modal">
    <div class="modal-title">Add Email Connection</div>
    <form id="add-connection-form">
      <div class="form-row">
        <div class="form-group">
          <label class="label">Label</label>
          <input class="input" type="text" name="label" placeholder="e.g. Work Email" required>
        </div>
        <div class="form-group">
          <label class="label">Provider</label>
          <select class="input" name="provider" id="conn-provider">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input class="input" type="text" name="imap_host" id="conn-host" value="imap.gmail.com">
        </div>
        <div class="form-group">
          <label class="label">IMAP Port</label>
          <input class="input" type="number" name="imap_port" id="conn-port" value="993">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label class="label">Email Address</label>
          <input class="input" type="email" name="imap_username" placeholder="you@example.com" required>
        </div>
        <div class="form-group">
          <label class="label">App Password</label>
          <input class="input" type="password" name="imap_password" placeholder="16-character app password" required>
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input class="input" type="number" name="scan_interval" value="6" min="1" max="168">
        </div>
      </div>

      <div class="expandable-header" id="guide-gmail-header">
        <span class="arrow">&#9654;</span> Gmail App Password Guide
      </div>
      <div class="expandable-body" id="guide-gmail-body">
        1. Go to <a href="https://myaccount.google.com/security" target="_blank" style="color:var(--blue)">myaccount.google.com/security</a><br>
        2. Enable 2-Step Verification if not already active<br>
        3. Go to <a href="https://myaccount.google.com/apppasswords" target="_blank" style="color:var(--blue)">App passwords</a><br>
        4. Select "Mail" and your device, then click Generate<br>
        5. Copy the 16-character password and paste it above<br>
        6. IMAP must be enabled in Gmail Settings &gt; Forwarding and POP/IMAP
      </div>
      <div class="expandable-header" id="guide-outlook-header">
        <span class="arrow">&#9654;</span> Outlook / Microsoft 365 App Password Guide
      </div>
      <div class="expandable-body" id="guide-outlook-body">
        1. Go to <a href="https://account.activedirectory.microsoft.com/" target="_blank" style="color:var(--blue)">account.activedirectory.microsoft.com</a><br>
        2. Sign in with your Microsoft 365 account<br>
        3. Go to Security info &gt; Add method &gt; App password<br>
        4. Name it (e.g. "SENTINEL") and click Add<br>
        5. Copy the generated password and paste it above<br>
        6. Ensure Basic Authentication is enabled for IMAP in your org admin
      </div>

      <div class="modal-actions">
        <button class="btn" type="button" data-action="close-modal">Cancel</button>
        <button class="btn btn-red" type="submit">Connect</button>
      </div>
    </form>
  </div>
</div>
...
(function(){
  ...
  window.onerror = function(msg, src, line, col, err) {
    showToast("JS Error: " + msg + " (" + (line||"?") + ":" + (col||"?") + ")", "error");
  };
  ...
  function loadTab(tab){
    switch(tab){
      ...
      case "team": loadTeam(); loadInvites(); break;
  ...
  function setLoading(el, msg){
    el.innerHTML = '<div class="loading-center"><div class="spinner"></div>' + (msg||"Loading...") + '</div>';
  }
  ...
  function statusBadge(s){
    var map = {completed:"green",success:"green",running:"blue",active:"blue",pending:"yellow",failed:"red",error:"red",disabled:"gray",sent:"blue",ended:"gray",revoked:"red",pending_invite:"yellow"};
    return '<span class="badge badge-'+(map[s]||"gray")+'">'+escHtml(s||"unknown")+'</span>';
  }
  ...
  function loadConnections(){
    ...
    setLoading(el);
    apiGet("/connections").then(function(d){
      var list = d.connections || [];
      if(!list.length){ el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">[ ]</div><div class="empty-state-text">No email connections yet. Add one to get started.</div></div>'; return; }
      var h = "";
      list.forEach(function(c){
        h += '<div class="conn-item">';
        h += '<div class="conn-dot '+(c.status==="active"?"online":"offline")+'"></div>';
        h += '<div class="conn-info">';
        h += '<div class="conn-label">'+escHtml(c.label||"Unnamed")+'</div>';
        h += '<div class="conn-detail">'+escHtml(c.imap_username||"")+' @ '+escHtml(c.imap_host||"")+':'+escHtml(String(c.imap_port||""))+'</div>';
        h += '<div style="font-size:11px;color:var(--dim);margin-top:2px">Last scan: '+formatDate(c.last_scan)+' &middot; Folder: '+escHtml(c.folder||"INBOX")+' &middot; Interval: every '+escHtml(String(c.scan_interval||6))+'h</div>';
        h += '</div>';
        h += '<div class="conn-actions">';
        h += '<button class="btn btn-sm btn-green" data-action="scan-now" data-id="'+escapeAttr(c.id)+'">Scan Now</button>';
        h += '<button class="btn btn-sm" style="color:var(--red)" data-action="remove-connection" data-id="'+escapeAttr(c.id)+'">Remove</button>';
        h += '</div>';
        h += '</div>';
      });
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  }
  ...
  document.getElementById("conn-provider").addEventListener("change", function(){
    var p = providerPresets[this.value];
    document.getElementById("conn-host").value = p.host;
    document.getElementById("conn-port").value = p.port;
  });

  document.getElementById("guide-gmail-header").addEventListener("click", function(){
    this.classList.toggle("open");
    document.getElementById("guide-gmail-body").classList.toggle("open");
  });
  document.getElementById("guide-outlook-header").addEventListener("click", function(){
    this.classList.toggle("open");
    document.getElementById("guide-outlook-body").classList.toggle("open");
  });

  document.getElementById("add-connection-form").addEventListener("submit", function(e){
    e.preventDefault();
    var fd = new FormData(this);
    var body = {};
    fd.forEach(function(v,k){ body[k]=v; });
    body.imap_port = parseInt(body.imap_port)||993;
    body.scan_interval = parseInt(body.scan_interval)||6;
    apiPost("/connections", body).then(function(){
      showToast("Connection added successfully");
      document.getElementById("modal-add-connection").classList.remove("open");
      document.getElementById("add-connection-form").reset();
      document.getElementById("conn-host").value="imap.gmail.com";
      document.getElementById("conn-port").value="993";
      loadConnections();
  ...
  function loadScans(){
    ...
    setLoading(el);
    apiGet("/scans").then(function(d){
      var list = d.jobs || [];
      if(!list.length){ el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">[S]</div><div class="empty-state-text">No scans have been run yet.</div></div>'; return; }
      var h = "";
      list.forEach(function(s){
        h += '<div class="scan-item">';
        h += '<div class="scan-top">';
        h += statusBadge(s.status);
        h += '<span style="font-size:12px;color:var(--dim)">'+formatDate(s.created_at)+'</span>';
        h += '</div>';
        h += '<div class="scan-stats">';
        h += '<span>Emails found: <b>'+escHtml(String(s.emails_found||0))+'</b></span>';
        h += '<span>Analyzed: <b>'+escHtml(String(s.emails_analyzed||0))+'</b></span>';
        if(s.error){ h += '<span style="color:var(--red)">Error: '+escHtml(s.error)+'</span>'; }
        h += '</div>';
        h += '</div>';
      });
      el.innerHTML = h;
  ...
  // TEAM
  function loadTeam(){
    var el = document.getElementById("team-members");
    setLoading(el);
    apiGet("/team").then(function(d){
      var list = d.members || [];
      if(!list.length){ el.innerHTML = '<div class="empty-state"><div class="empty-state-text">No team members.</div></div>'; return; }
      var h = "";
      list.forEach(function(m){
        var initial = (m.username||m.email||"?").charAt(0).toUpperCase();
        h += '<div class="member-item">';
        h += '<div class="member-avatar">'+escHtml(initial)+'</div>';
        h += '<div class="member-info">';
        h += '<div class="member-name">'+escHtml(m.username||"Unknown")+'</div>';
        h += '<div class="member-email">'+escHtml(m.email||"")+'</div>';
        h += '<div class="member-role">Role: '+escHtml(m.role||"member")+'</div>';
        h += '</div>';
        h += '<button class="btn btn-sm" style="color:var(--red)" data-action="remove-member" data-id="'+escapeAttr(m.id)+'">Remove</button>';
        h += '</div>';
      });
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  ...
  function loadInvites(){
    var el = document.getElementById("team-invites");
    setLoading(el);
    apiGet("/invites").then(function(d){
      var list = d.invites || [];
      if(!list.length){ el.innerHTML = '<div class="empty-state"><div class="empty-state-text">No pending invites.</div></div>'; return; }
      var h = "";
      list.forEach(function(inv){
        h += '<div class="invite-item">';
        h += '<div class="conn-info">';
        h += '<div class="conn-label">'+escHtml(inv.email)+'</div>';
        h += '<div class="conn-detail">Role: '+escHtml(inv.role||"member")+' &middot; '+statusBadge(inv.status||"pending_invite")+'</div>';
        if(inv.invite_link){ h += '<div style="margin-top:4px"><button class="btn btn-sm" data-action="copy-invite" data-link="'+escapeAttr(inv.invite_link)+'">Copy Invite Link</button></div>'; }
        h += '</div>';
        h += '<button class="btn btn-sm" style="color:var(--red)" data-action="revoke-invite" data-id="'+escapeAttr(inv.id)+'">Revoke</button>';
        h += '</div>';
      });
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  ...
  document.getElementById("invite-form").addEventListener("submit", function(e){
    e.preventDefault();
    var fd = new FormData(this);
    var body = {email:fd.get("email"), role:fd.get("role")};
    apiPost("/invites", body).then(function(){
  ...
  function loadReports(){
    ...
    setLoading(el);
    apiGet("/report-schedule").then(function(d){
      var enabled = d.enabled || false;
      var freq = d.frequency || "weekly";
      var recipients = d.recipients || "";
      var h = "";
      h += '<div class="toggle-row">';
      h += '<label>Enable Reports</label>';
      h += '<label class="toggle-switch"><input type="checkbox" id="report-enabled"'+(enabled?" checked":"")+'><span class="slider"></span></label>';
      h += '</div>';
      h += '<div class="form-row">';
      h += '<div class="form-group"><label class="label">Frequency</label><select class="input" id="report-freq"><option value="weekly"'+(freq==="weekly"?" selected":"")+'>Weekly</option><option value="monthly"'+(freq==="monthly"?" selected":"")+'>Monthly</option></select></div>';
      h += '<div class="form-group"><label class="label">Recipients (comma separated)</label><input class="input" type="text" id="report-recipients" value="'+escHtml(recipients)+'" placeholder="user@example.com, admin@example.com"></div>';
      h += '</div>';
      h += '<div style="display:flex;gap:8px;margin-top:8px">';
      h += '<button class="btn btn-red" data-action="save-reports">Save Schedule</button>';
      h += '<button class="btn" data-action="test-report">Download Test Report</button>';
      h += '</div>';
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  }
  ...
  function loadBranding(){
    apiGet("/branding").then(function(d){
      var f = document.getElementById("branding-form");
      if(d.org_display_name) f.elements["org_display_name"].value = d.org_display_name;
      if(d.logo_url) f.elements["logo_url"].value = d.logo_url;
      if(d.primary_color) f.elements["primary_color"].value = d.primary_color;
      if(d.secondary_color) f.elements["secondary_color"].value = d.secondary_color;
      if(d.custom_css) f.elements["custom_css"].value = d.custom_css;
      document.getElementById("primary-color-val").textContent = d.primary_color || "#DC2626";
      document.getElementById("secondary-color-val").textContent = d.secondary_color || "#111111";
      updateBrandingPreview(d.logo_url);
    }).catch(function(){});
  }

  document.getElementById("branding-form").querySelector('[name="primary_color"]').addEventListener("input", function(){
    document.getElementById("primary-color-val").textContent = this.value;
  });
  document.getElementById("branding-form").querySelector('[name="secondary_color"]').addEventListener("input", function(){
    document.getElementById("secondary-color-val").textContent = this.value;
  });
  document.getElementById("branding-form").querySelector('[name="logo_url"]').addEventListener("input", function(){
    updateBrandingPreview(this.value);
  });

  function updateBrandingPreview(url){
    var box = document.getElementById("branding-preview");
    var img = document.getElementById("branding-preview-img");
    if(url){ box.style.display="block"; img.src=url; }
    else { box.style.display="none"; }
  }

  document.getElementById("branding-preview-img").addEventListener("error", function(){ this.remove(); });
  ...
  document.getElementById("branding-form").addEventListener("submit", function(e){
    ...
    var body = {};
    fd.forEach(function(v,k){ body[k]=v; });
    apiPost("/branding", body).then(function(){
  ...
  function loadRules(){
    ...
    setLoading(el);
    apiGet("/rules").then(function(d){
      var list = d.rules || [];
      if(!list.length){ el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">[L]</div><div class="empty-state-text">No detection rules configured yet.</div></div>'; return; }
      var h = "";
      list.forEach(function(r){
        h += '<div class="rule-item">';
        h += '<div class="rule-info">';
        h += '<div class="rule-name">'+escHtml(r.name)+'</div>';
        h += '<div class="rule-pattern">'+escHtml(r.pattern)+'</div>';
        h += '<div class="rule-badges">';
        h += '<span class="badge badge-blue">'+escHtml(r.rule_type||"keyword")+'</span>';
        h += '<span class="badge badge-'+(r.action==="flag"?"yellow":r.action==="block"?"red":"green")+'">'+escHtml(r.action||"flag")+'</span>';
        if(r.description){ h += '<span class="badge badge-gray">'+escHtml(r.description)+'</span>'; }
        h += '</div>';
        h += '</div>';
        h += '<label class="rule-toggle"><input type="checkbox" data-action="toggle-rule" data-id="'+escapeAttr(r.id)+'"'+(r.enabled!==false?" checked":"")+'><span class="slider"></span></label>';
        h += '<button class="btn btn-sm" style="color:var(--red)" data-action="delete-rule" data-id="'+escapeAttr(r.id)+'">Delete</button>';
        h += '</div>';
      });
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  ...
  document.getElementById("rule-form").addEventListener("submit", function(e){
    ...
    var body = {};
    fd.forEach(function(v,k){ body[k]=v; });
    apiPost("/rules", body).then(function(){
  ...
  function loadAlerts(){
    ...
    setLoading(el);
    apiGet("/alerts/config").then(function(d){
      var enabled = d.enabled || false;
      var platform = d.platform || "slack";
      var alertLevel = d.alert_level || "malicious";
      var webhook = d.webhook_url || "";
      var h = "";
      h += '<div class="toggle-row">';
      h += '<label>Enable Alerts</label>';
      h += '<label class="toggle-switch"><input type="checkbox" id="alert-enabled"'+(enabled?" checked":"")+'><span class="slider"></span></label>';
      h += '</div>';
      h += '<div class="form-row">';
      h += '<div class="form-group"><label class="label">Platform</label><select class="input" id="alert-platform"><option value="slack"'+(platform==="slack"?" selected":"")+'>Slack</option><option value="discord"'+(platform==="discord"?" selected":"")+'>Discord</option><option value="teams"'+(platform==="teams"?" selected":"")+'>Microsoft Teams</option></select></div>';
      h += '<div class="form-group"><label class="label">Alert Level</label><select class="input" id="alert-level"><option value="malicious"'+(alertLevel==="malicious"?" selected":"")+'>Malicious Only</option><option value="suspicious"'+(alertLevel==="suspicious"?" selected":"")+'>Suspicious and Above</option></select></div>';
      h += '</div>';
      h += '<div class="form-group"><label class="label">Webhook URL</label><input class="input" type="url" id="alert-webhook" value="'+escHtml(webhook)+'" placeholder="https://hooks.slack.com/services/..."></div>';
      h += '<div style="display:flex;gap:8px;margin-top:8px">';
      h += '<button class="btn btn-red" data-action="save-alerts">Save Config</button>';
      h += '<button class="btn" data-action="test-alert">Send Test Alert</button>';
      h += '</div>';
      el.innerHTML = h;
    }).catch(function(e){ el.innerHTML = '<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  }
  ...
  // INTEL
  function loadIntel(){
    apiGet("/intel/stats").then(function(d){
      var el = document.getElementById("intel-stats");
      var bt = d.by_type || {};
      var h = '<div class="stat-grid">';
      h += '<div class="stat-card"><div class="stat-value">'+escHtml(String(d.total_indicators||0))+'</div><div class="stat-label">Total Indicators</div></div>';
      h += '<div class="stat-card"><div class="stat-value">'+escHtml(String(d.verified||0))+'</div><div class="stat-label">Verified</div></div>';
      h += '<div class="stat-card"><div class="stat-value">'+escHtml(String(bt.domain||0))+'</div><div class="stat-label">Domains</div></div>';
      h += '<div class="stat-card"><div class="stat-value">'+escHtml(String(bt.url||0))+'</div><div class="stat-label">URLs</div></div>';
      h += '</div>';
      if(d.top_reported && d.top_reported.length){
        h += '<div style="margin-top:4px"><div class="label" style="margin-bottom:8px">Top Reported</div>';
        d.top_reported.forEach(function(t){ h += '<div style="font-size:13px;padding:4px 0;color:var(--dim)">'+escHtml(t.indicator||t)+'</div>'; });
        h += '</div>';
      }
      el.innerHTML = h;
    }).catch(function(e){ document.getElementById("intel-stats").innerHTML='<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });

    var feedEl = document.getElementById("intel-feed");
    setLoading(feedEl);
    apiGet("/intel").then(function(d){
      var list = d.items || [];
      if(!list.length){ feedEl.innerHTML='<div class="empty-state"><div class="empty-state-text">No intel indicators yet.</div></div>'; return; }
      var h = "";
      list.forEach(function(item){
        h += '<div style="padding:10px 0;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px">';
        h += '<div style="flex:1;min-width:0">';
        h += '<div style="font-size:14px;font-family:var(--mono)">'+escHtml(item.indicator)+'</div>';
        h += '<div style="margin-top:4px"><span class="badge badge-blue">'+escHtml(item.indicator_type||"")+'</span> <span class="badge badge-purple">'+escHtml(item.category||"")+'</span></div>';
        h += '</div>';
        h += '<span style="font-size:11px;color:var(--dim)">'+formatDate(item.created_at)+'</span>';
        h += '</div>';
      });
      feedEl.innerHTML = h;
    }).catch(function(e){ feedEl.innerHTML='<div class="empty-state" style="color:var(--red)">'+escHtml(e.message)+'</div>'; });
  }

  document.getElementById("intel-form").addEventListener("submit", function(e){
    e.preventDefault();
    var fd = new FormData(this);
    var body = {};
    fd.forEach(function(v,k){ body[k]=v; });
    apiPost("/intel", body).then(function(){
      showToast("Indicator submitted");
  ...
  function loadSimulations(){
    ...
    setLoading(el);
    apiGet("/simulations").then(function(d){
      var list = d.simulations || [];
      if(!list.length){ el.innerHTML='<div class="empty-state"><div class="empty-state-icon">[X]</div><div class="empty-state-text">No simulations created yet.</div></div>'; return; }
      var h = "";
      list.forEach(function(s){
        var stats = s.stats || {};
        ...
        h += '<div class="sim-top">';
        h += '<div style="flex:1">';
        h += '<span style="font-weight:600;font-size:14px">'+escHtml(s.name)+'</span>';
        h += ' '+statusBadge(s.status);
        h += '</div>';
        h += '<span style="font-size:12px;color:var(--dim)">'+formatDate(s.created_at)+'</span>';
        h += '</div>';
        h += '<div style="font-size:12px;color:var(--dim);margin-top:2px">Template: '+escHtml(s.template_type||"")+'</div>';
        h += '<div class="sim-stats">';
        h += '<span>Sent: <b>'+escHtml(String(stats.sent||0))+'</b></span>';
        h += '<span>Opened: <b>'+escHtml(String(stats.opened||0))+'</b></span>';
        h += '<span>Clicked: <b>'+escHtml(String(stats.clicked||0))+'</b></span>';
        h += '<span>Credentials: <b>'+escHtml(String(stats.credentials||0))+'</b></span>';
        if(stats.risk_score !== undefined){ h += '<span>Risk: <b style="color:var(--red)">'+escHtml(String(stats.risk_score))+'</b></span>'; }
        h += '</div>';
        h += '<div style="margin-top:8px;display:flex;gap:6px">';
        if(s.status==="created"||s.status==="draft"){
          h += '<button class="btn btn-sm btn-green" data-action="send-sim" data-id="'+escapeAttr(s.id)+'">Send</button>';
        }
        if(s.status==="sent"||s.status==="active"||s.status==="running"){
          h += '<button class="btn btn-sm" style="color:var(--yellow)" data-action="end-sim" data-id="'+escapeAttr(s.id)+'">End</button>';
        }
        h += '</div>';
        h += '</div>';
      });
  ...
  document.getElementById("sim-form").addEventListener("submit", function(e){
    ...
    if(body.target_user_ids){
      body.target_user_ids = body.target_user_ids.split(",").map(function(s){return s.trim()}).filter(Boolean);
    }
    apiPost("/simulations", body).then(function(){
  ...
  document.addEventListener("click", function(e){
    ...
    switch(action){
      ...
      case "open-add-connection": document.getElementById("modal-add-connection").classList.add("open"); break;
      case "close-modal": t.closest(".modal-overlay").classList.remove("open"); break;
      case "scan-now":
        showToast("Starting scan...");
        apiPost("/scan/"+id, {}).then(function(){ showToast("Scan started"); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "remove-connection":
        if(confirm("Remove this connection?")){
          apiDel("/connections/"+id).then(function(){ showToast("Connection removed"); loadConnections(); }).catch(function(e){ showToast(e.message, "error"); });
        }
        break;
      case "save-reports":
        var repBody = {
        ...
        apiPost("/report-schedule", repBody).then(function(){ showToast("Report schedule saved"); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "test-report":
        showToast("Generating test report...");
        fetch("/api/v1/report-schedule/test", {method:"POST", headers:{"Authorization":"Bearer "+getToken()}}).then(function(r){
          ...
          a.href = url;
          a.download = "sentinel_test_report.pdf";
          a.click();
          URL.revokeObjectURL(url);
          showToast("Report downloaded");
        }).catch(function(e){ showToast("Failed to download report", "error"); });
        break;
      case "save-alerts":
        var alertBody = {
          enabled: document.getElementById("alert-enabled").checked,
          platform: document.getElementById("alert-platform").value,
          alert_level: document.getElementById("alert-level").value,
          webhook_url: document.getElementById("alert-webhook").value
        };
        apiPost("/alerts/config", alertBody).then(function(){ showToast("Alert config saved"); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "test-alert":
        showToast("Sending test alert...");
        apiPost("/alerts/test", {}).then(function(){ showToast("Test alert sent"); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "copy-invite":
        var link = t.dataset.link;
        if(navigator.clipboard){ navigator.clipboard.writeText(link).then(function(){ showToast("Invite link copied"); }); }
        else { var ta=document.createElement("textarea");ta.value=link;document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);showToast("Invite link copied"); }
        break;
      case "revoke-invite":
        apiDel("/invites/"+id).then(function(){ showToast("Invite revoked"); loadInvites(); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "remove-member":
        if(confirm("Remove this team member?")){
          apiDel("/team/"+id).then(function(){ showToast("Member removed"); loadTeam(); }).catch(function(e){ showToast(e.message, "error"); });
        }
        break;
      case "delete-rule":
        if(confirm("Delete this rule?")){
          apiDel("/rules/"+id).then(function(){ showToast("Rule deleted"); loadRules(); }).catch(function(e){ showToast(e.message, "error"); });
        }
        break;
      case "test-rules":
        var text = document.getElementById("rule-test-input").value;
        if(!text){ showToast("Enter text to test", "error"); return; }
        var resEl = document.getElementById("rule-test-result");
        resEl.innerHTML = '<div class="loading-center"><div class="spinner"></div>Testing...</div>';
        apiPost("/rules/check", {text:text}).then(function(d){
          var matches = d.matches || d.rules || [];
          if(!matches.length){ resEl.innerHTML='<div style="padding:12px;color:var(--dim);font-size:13px">No rules matched.</div>'; return; }
          var h = '<div style="margin-top:12px">';
          matches.forEach(function(m){
            h += '<div style="padding:8px 12px;background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.2);border-radius:6px;margin-bottom:6px;font-size:13px">';
            h += '<b>'+escHtml(m.name||"Rule")+'</b> &mdash; ';
            h += '<span class="badge badge-'+(m.action==="flag"?"yellow":m.action==="block"?"red":"green")+'">'+escHtml(m.action||"")+'</span>';
            if(m.description){ h += ' <span style="color:var(--dim)">'+escHtml(m.description)+'</span>'; }
            h += '</div>';
          });
          h += '</div>';
          resEl.innerHTML = h;
        }).catch(function(e){ resEl.innerHTML='<div style="padding:12px;color:var(--red);font-size:13px">'+escHtml(e.message)+'</div>'; });
        break;
      case "reset-branding":
        ...
        break;
      case "send-sim":
        apiPost("/simulations/"+id+"/send", {}).then(function(){ showToast("Simulation sent"); loadSimulations(); }).catch(function(e){ showToast(e.message, "error"); });
        break;
      case "end-sim":
        apiPost("/simulations/"+id+"/end", {}).then(function(){ showToast("Simulation ended"); loadSimulations(); }).catch(function(e){ showToast(e.message, "error"); });
        break;
    }
  });
  ...
  document.addEventListener("change", function(e){
    ...
    if(action === "toggle-rule"){
      apiPatch("/rules/"+id+"/toggle", {}).then(function(){ showToast("Rule toggled"); }).catch(function(e){ showToast(e.message, "error"); });
    }
  });

  document.getElementById("modal-add-connection").addEventListener("click", function(e){
